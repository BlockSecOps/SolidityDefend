...
pub enum TypeName<'arena> {
    ...
    Array {
        base_type: Box<TypeName<'arena>>,
        length: Option<Box<Expression<'arena>>>,
    },
    Mapping {
        key_type: Box<TypeName<'arena>>,
        value_type: Box<TypeName<'arena>>,
    },
    Function {
...
pub enum Statement<'arena> {
    ...
    If {
        ...
        then_branch: Box<Statement<'arena>>,
        else_branch: Option<Box<Statement<'arena>>>,
        location: SourceLocation,
    },
    While {
        condition: Expression<'arena>,
    ...
    For {
        init: Option<Box<Statement<'arena>>>,
...
pub enum Expression<'arena> {
    ...
    BinaryOperation {
        left: Box<Expression<'arena>>,
        operator: BinaryOperator,
        right: Box<Expression<'arena>>,
        location: SourceLocation,
    },
    UnaryOperation {
        operator: UnaryOperator,
        operand: Box<Expression<'arena>>,
        prefix: bool,
        location: SourceLocation,
    },
    Assignment {
    ...
    MemberAccess {
        expression: Box<Expression<'arena>>,
        member: Identifier<'arena>,
        location: SourceLocation,
    },
    IndexAccess {
        base: Box<Expression<'arena>>,
        index: Option<Box<Expression<'arena>>>,
        location: SourceLocation,
    },
    FunctionCall {
    ...
    NewExpression {
        type_name: Box<TypeName<'arena>>,
        location: SourceLocation,
    },
    Conditional {
        condition: Box<Expression<'arena>>,
        true_expression: Box<Expression<'arena>>,
        false_expression: Box<Expression<'arena>>,
        location: SourceLocation,
    },
    TypeCast {
        type_name: Box<TypeName<'arena>>,
