...
impl Erc777ReentrancyHooksDetector {
    fn check_erc777_reentrancy(&self, function: &ast::Function<'_>, ctx: &AnalysisContext) -> Option<String> {
        ...
        let has_token_transfer = func_source.contains(".transfer(") ||
                                func_source.contains(".transferFrom(");

        // Check for ERC-1820 registry (used by ERC-777)
        let has_erc1820 = func_source.contains("ERC1820") ||
                         func_source.contains("IERC1820Registry");
        ...
        if (has_erc777_ops || (has_token_transfer && has_erc1820)) && !has_reentrancy_guard {
            ...
            if state_after_transfer {
                return Some("State changes after ERC-777 transfer without reentrancy guard".to_string());
            }
        ...
        if function.name.name == "tokensReceived" || function.name.name == "tokensToSend" {
            ...
            if has_external_calls && !has_reentrancy_guard {
                return Some(format!("Callback '{}' makes external calls without reentrancy guard", function.name.name));
            }
        }
    ...
    fn has_state_change_after_transfer(&self, source: &str) -> bool {
        ...
        for line in lines {
            ...
            if found_transfer && (line.contains(" = ") || line.contains("+=") || line.contains("-=")) {
                return true;
            }
        }
