            ...
            if trimmed.contains("function") &&
               (trimmed.contains("setEntryPoint") ||
                ...
                if !has_owner_check {
                    ...
                    issues.push((
                        (idx + 1) as u32,
                        "EntryPoint replacement lacks time-lock or multi-sig protection".to_string()
                    ));
                }
            }
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if trimmed.contains("function") &&
               (trimmed.contains("addModule") ||
                ...
                let has_validation = func_body.contains("require(") ||
                ...
                let has_signature = func_body.contains("signature") ||
    ...
    fn check_signature_bypass(&self, source: &str) -> Option<Vec<(u32, String)>> {
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if trimmed.contains("function validateUserOp") {
                ...
                let validates_owner = func_body.contains("owner") ||
                                     func_body.contains("signer");
                ...
                if !has_sig_check {
                    ...
                    issues.push((
                        (idx + 1) as u32,
                        "validateUserOp has early return that may bypass signature check".to_string()
                    ));
                }
            }
        }
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if trimmed.contains("function") &&
               (trimmed.contains("upgradeTo") ||
                ...
                let has_owner = func_body.contains("onlyOwner") ||
                               func_body.contains("require(owner");

                let has_timelock = func_body.contains("timelock") ||
                                  func_body.contains("delay");

                let validates_impl = func_body.contains("require(") &&
                                    func_body.contains("!= address(0)");
...
mod tests {
    ...
    fn test_detector_properties() {
        ...
        assert_eq!(detector.name(), "Account Abstraction Takeover Vulnerability");
