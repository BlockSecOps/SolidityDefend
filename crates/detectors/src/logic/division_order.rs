...
impl DivisionOrderDetector {
    ...
    fn analyze_expression_for_division_order(&self, expr: &ast::Expression<'_>, ctx: &AnalysisContext<'_>) -> Vec<Finding> {
        ...
        match expr {
            ...
            ast::Expression::Assignment { operator, left: _, right, location } => {
                match operator {
                    ast::AssignmentOperator::MulAssign => {
                        // Check for division in right operand of *=
                        if self.contains_division(right) {
                            let message = "Division before multiplication assignment detected".to_string();
                            let finding = self.base.create_finding(
                                ctx,
                                message,
                                location.start().line() as u32,
                    ...
                    ast::AssignmentOperator::DivAssign => {
                        ...
                        if self.contains_division(right) {
                            let message = "Multiple division operations detected in assignment".to_string();
                            let finding = self.base.create_finding(
                                ctx,
                                message,
                                location.start().line() as u32,
            ...
            ast::Expression::FunctionCall { function, arguments, location, .. } => {
                ...
                if let ast::Expression::Identifier(id) = function {
                    if id.name.to_lowercase().contains("mul") && arguments.len() >= 2 {
                        if self.contains_division(&arguments[0]) {
                            let message = "Division in first argument of multiplication function".to_string();
                            let finding = self.base.create_finding(
                                ctx,
                                message,
                                location.start().line() as u32,
    ...
    fn contains_division(&self, expr: &ast::Expression<'_>) -> bool {
        match expr {
            ...
            ast::Expression::Assignment { operator, right, .. } => {
    ...
    fn analyze_return_statement(&self, stmt: &ast::Statement<'_>, ctx: &AnalysisContext<'_>) -> Vec<Finding> {
        let mut findings = Vec::new();

        if let ast::Statement::Return { value: Some(expr), location } = stmt {
            // Check for division before multiplication in return expressions
            if let ast::Expression::BinaryOperation { operator: ast::BinaryOperator::Mul, left, .. } = expr {
                if self.contains_division(left) {
                    let message = "Return statement contains division before multiplication".to_string();
                    let finding = self.base.create_finding(
                        ctx,
                        message,
                        location.start().line() as u32,
                    ...
                    .with_cwe(682)
                    .with_fix_suggestion(
                        "Consider reordering operations or using higher precision arithmetic".to_string()
                    );
                    findings.push(finding);
                }
            }
    ...
    fn analyze_loop_for_precision_loss(&self, stmt: &ast::Statement<'_>, ctx: &AnalysisContext<'_>) -> Vec<Finding> {
        let mut findings = Vec::new();

        match stmt {
            ast::Statement::For { update: Some(update), body, location, .. } => {
    ...
    fn contains_division_before_multiplication(&self, expr: &ast::Expression<'_>) -> bool {
        match expr {
            ast::Expression::BinaryOperation { operator: ast::BinaryOperator::Mul, left, .. } => {
                self.contains_division(left)
            }
            ast::Expression::Assignment { operator: ast::AssignmentOperator::MulAssign, right, .. } => {
                self.contains_division(right)
            }
            _ => false,
        }
...
impl AstAnalyzer for DivisionOrderDetector {
    ...
    fn analyze_statement(&self, statement: &ast::Statement<'_>, ctx: &AnalysisContext<'_>) -> Result<Vec<Finding>> {
        ...
        match statement {
            ...
            ast::Statement::TryStatement { body, catch_clauses, .. } => {
                for stmt in &body.statements {
                    findings.extend(self.analyze_statement(stmt, ctx)?);
                }
