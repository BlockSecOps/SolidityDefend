...
impl Detector for CrossChainMessageOrderingDetector {
    ...
    fn detect(&self, ctx: &AnalysisContext<'_>) -> Result<Vec<Finding>> {
        ...
        if lower.contains("relayer") || lower.contains("crosschain") {
            if !lower.contains("sequence") && !lower.contains("nonce") {
                findings.push(self.base.create_finding(
                    ctx,
                    "Cross-chain messages lack sequence/nonce - ordering not guaranteed".to_string(),
                    1, 1, ctx.source_code.len() as u32,
                ).with_fix_suggestion("Add sequence number: mapping(bytes32 => uint256) public messageNonce".to_string()));
            }
        }
        Ok(findings)
    }
